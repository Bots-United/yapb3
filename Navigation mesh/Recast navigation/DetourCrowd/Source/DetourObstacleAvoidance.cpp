					   ...
					   float& t)
{
	...
	if (Math::fabsf(d) < 1e-6f) return 0;
	d = 1.0f/d;
	t = dtVperp2D(v,w) * d;
	if (t < 0 || t > 1) return 0;
...
static void normalizeArray(float* arr, const int n)
{
	...
	float minPen = Math::MaximumFloatValue;
	float maxPen = -Math::MaximumFloatValue;
	for (int i = 0; i < n; ++i)
	{
												 ...
												 dtObstacleAvoidanceDebugData* debug)
{
	...
	float minPenalty = Math::MaximumFloatValue;
	int ns = 0;
													 ...
													 dtObstacleAvoidanceDebugData* debug)
{
	...
	const float dang = Math::atan2f(dvel[2], dvel[0]);
	...
	for (int j = 0; j < nr; ++j)
	{
		...
		for (int i = 0; i < nd; ++i)
		{
			pat[npat*2+0] = Math::cosf(a)*r;
			pat[npat*2+1] = Math::sinf(a)*r;
			npat++;
			a += da;
		}
	}
