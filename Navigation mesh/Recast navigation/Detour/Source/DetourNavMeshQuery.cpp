...
*/
	float closestDistSqr = Math::MaximumFloatValue;
	for (int j = 0; j < pd->triCount; ++j)
	{
		const unsigned char* t = &tile->detailTris[(pd->triBase+j)*4];
		const float* v[3];
										 ...
										 dtPolyRef* nearestRef, float* nearestPt) const
{
	...
	*nearestRef = 0;
	...
	dtPolyRef nearest = 0;
	float nearestDistanceSqr = Math::MaximumFloatValue;
...
/// 
/// If the hit parameter is a very high value (Math::MaximumFloatValue), then the ray has hit 
...
/// If it reaches the end position's xz-coordinates it will indicate Math::MaximumFloatValue
