						...
						const int /*nnodes*/, dtBVNode* nodes)
{
	...
	for (int i = 0; i < npolys; i++)
	{
		...
		// Remap y
		it.bmin[1] = (unsigned short)Math::floorf((float)it.bmin[1]*ch/cs);
		it.bmax[1] = (unsigned short)Math::ceilf((float)it.bmax[1]*ch/cs);
	}
...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	{
		HalfLifeEngine::Globals::g_halfLifeEngine->ServerPrintFormat ("dtCreateNavMeshData(): params->nvp(%u) > (%u)DT_VERTS_PER_POLYGON\n", params->nvp, DT_VERTS_PER_POLYGON);

		return false;
	}
	if (params->vertCount >= 0xffff)
	{
		HalfLifeEngine::Globals::g_halfLifeEngine->ServerPrintFormat ("dtCreateNavMeshData(): params->vertCount(%u) >= %u\n", params->vertCount, 0xffffu);

		return false;
	}
	if (!params->vertCount || !params->verts)
	{
		HalfLifeEngine::Globals::g_halfLifeEngine->ServerPrint (!params->vertCount ? "dtCreateNavMeshData(): params->vertCount == 0\n" : "dtCreateNavMeshData(): params->verts == NULL\n");

		return false;
	}
	if (!params->polyCount || !params->polys)
	{
		HalfLifeEngine::Globals::g_halfLifeEngine->ServerPrint (!params->polyCount ? "dtCreateNavMeshData(): params->polyCount == 0\n" : "dtCreateNavMeshData(): params->polys == NULL\n");
	...
	if (params->offMeshConCount > 0)
	{
		...
		if (!offMeshConClass)
		{
			HalfLifeEngine::Globals::g_halfLifeEngine->ServerPrint ("dtCreateNavMeshData(): !offMeshConClass\n");

			return false;
		}

		// Find tight heigh bounds, used for culling out off-mesh start locations.
		float hmin = Math::MaximumFloatValue;
		float hmax = -Math::MaximumFloatValue;
	...
	if (!data)
	{
		HalfLifeEngine::Globals::g_halfLifeEngine->ServerPrint ("dtCreateNavMeshData(): data == NULL\n");
