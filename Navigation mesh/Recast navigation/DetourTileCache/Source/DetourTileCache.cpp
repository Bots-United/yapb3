...
{
	inline BuildContext_(struct dtTileCacheAlloc* a) : layer(0), lcset(0), lmesh(0), alloc(a) {}
	inline ~BuildContext_() { purge(); }
	void purge()
	{
								 ...
								 dtCompressedTileRef* results, int* resultCount, const int maxResults) const 
{
	...
	const int tx0 = (int)Math::floorf((bmin[0]-m_params.orig[0]) / tw);
	const int tx1 = (int)Math::floorf((bmax[0]-m_params.orig[0]) / tw);
	const int ty0 = (int)Math::floorf((bmin[2]-m_params.orig[2]) / th);
	const int ty1 = (int)Math::floorf((bmax[2]-m_params.orig[2]) / th);
