...
void Sample_TileMesh::handleSettings()
{
	...
	char msg[64];
	_snprintf(msg, 64, "Build Time: %.1fms", m_totalBuildTimeMs);
	imguiLabel(msg);
...
void Sample_TileMesh::handleRenderOverlay(double* proj, double* model, int* view)
{
	...
	{
		char text[32];
		_snprintf(text,32,"%.3fms / %dTris / %.1fkB", m_tileBuildTime, m_tileTriCount, m_tileMemUsage);
		imguiDrawText((int)x, (int)y-25, IMGUI_ALIGN_CENTER, text, imguiRGBA(0,0,0,220));
	}
...
unsigned char* Sample_TileMesh::buildTileMesh(const int tx, const int ty, const float* bmin, const float* bmax, int& dataSize)
{
	...
	m_cfg.walkableHeight = (int)Math::ceilf(m_agentHeight / m_cfg.ch);
	m_cfg.walkableClimb = (int)Math::floorf(m_agentMaxClimb / m_cfg.ch);
	m_cfg.walkableRadius = (int)Math::ceilf(m_agentRadius / m_cfg.cs);
	...
	for (int i = 0; i < ncid; ++i)
	{
		...
		rcMarkWalkableTriangles(m_ctx, m_cfg.walkableSlopeAngle/*!,
								verts, nverts, ctris*/, nctris, m_geom->getMesh()->getNormals(), m_triareas);
