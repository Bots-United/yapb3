...
void TestCase::handleRender()
{
	...
	for (Test* iter = m_tests; iter; iter = iter->next)
	{
		...
		dtVnormalize(dir);
		g_drawInterface->glColor4ub(128,25,0,192);
		g_drawInterface->glVertex3f(iter->spos[0],iter->spos[1]-0.3f,iter->spos[2]);
		g_drawInterface->glVertex3f(iter->spos[0],iter->spos[1]+0.3f,iter->spos[2]);
		g_drawInterface->glVertex3f(iter->spos[0],iter->spos[1]+0.3f,iter->spos[2]);
		g_drawInterface->glVertex3f(iter->spos[0]+dir[0]*0.3f,iter->spos[1]+0.3f+dir[1]*0.3f,iter->spos[2]+dir[2]*0.3f);
		g_drawInterface->glColor4ub(51,102,0,129);
		g_drawInterface->glVertex3f(iter->epos[0],iter->epos[1]-0.3f,iter->epos[2]);
		g_drawInterface->glVertex3f(iter->epos[0],iter->epos[1]+0.3f,iter->epos[2]);
		
		if (iter->expand)
			g_drawInterface->glColor4ub(255,192,0,255);
		else
			g_drawInterface->glColor4ub(0,0,0,64);
			
		for (int i = 0; i < iter->nstraight-1; ++i)
		{
			g_drawInterface->glVertex3f(iter->straight[i*3+0],iter->straight[i*3+1]+0.3f,iter->straight[i*3+2]);
			g_drawInterface->glVertex3f(iter->straight[(i+1)*3+0],iter->straight[(i+1)*3+1]+0.3f,iter->straight[(i+1)*3+2]);
		}
	}
...
//		mouseOverMenu = true;
	...
	{
		const int total = iter->findNearestPolyTime + iter->findPathTime + iter->findStraightPathTime;
		_snprintf(subtext, 64, "%.4f ms", (float)total/1000.0f);
		_snprintf(text, 64, "Path %d", n);
		
		if (imguiCollapse(text, subtext, iter->expand))
			iter->expand = !iter->expand;
		if (iter->expand)
		{
			_snprintf(text, 64, "Poly: %.4f ms", (float)iter->findNearestPolyTime/1000.0f);
			imguiValue(text);

			_snprintf(text, 64, "Path: %.4f ms", (float)iter->findPathTime/1000.0f);
			imguiValue(text);

			_snprintf(text, 64, "Straight: %.4f ms", (float)iter->findStraightPathTime/1000.0f);
			imguiValue(text);
