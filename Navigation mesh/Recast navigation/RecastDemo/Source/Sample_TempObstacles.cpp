							   ...
							   const int maxTiles)
{
	...
	for (int i = 0; i < ncid; ++i)
	{
		...
		rcMarkWalkableTriangles(ctx, tcfg.walkableSlopeAngle/*!,
								verts, nverts, tris*/, ntris, geom->getMesh()->getNormals(), rc.triareas);
...
void drawDetailOverlay(const dtTileCache* tc, const int tx, const int ty, double* proj, double* model, int* view)
{
	...
	for (int i = 0; i < ntiles; ++i)
	{
		...
		{
			_snprintf(text,128,"(%d,%d)/%d", tile->header->tx,tile->header->ty,tile->header->tlayer);
			imguiDrawText((int)x, (int)y-25, IMGUI_ALIGN_CENTER, text, imguiRGBA(0,0,0,220));
			_snprintf(text,128,"Compressed: %.1f kB", tile->dataSize/1024.0f);
			imguiDrawText((int)x, (int)y-45, IMGUI_ALIGN_CENTER, text, imguiRGBA(0,0,0,128));
			_snprintf(text,128,"Raw:%.1fkB", rawSize/1024.0f);
...
{
	float tmin = Math::MaximumFloatValue;
...
public:
	...
	virtual void handleRender()
	{
		...
		if (m_hitPosSet)
		{
			...
			g_drawInterface->glColor4ub(0,0,0,128);
			g_drawInterface->glLineWidth(2.0f);
			g_drawInterface->glBegin(GL_LINES);
			g_drawInterface->glVertex3f(m_hitPos[0]-s,m_hitPos[1]+0.1f,m_hitPos[2]);
			g_drawInterface->glVertex3f(m_hitPos[0]+s,m_hitPos[1]+0.1f,m_hitPos[2]);
			g_drawInterface->glVertex3f(m_hitPos[0],m_hitPos[1]-s+0.1f,m_hitPos[2]);
			g_drawInterface->glVertex3f(m_hitPos[0],m_hitPos[1]+s+0.1f,m_hitPos[2]);
			g_drawInterface->glVertex3f(m_hitPos[0],m_hitPos[1]+0.1f,m_hitPos[2]-s);
			g_drawInterface->glVertex3f(m_hitPos[0],m_hitPos[1]+0.1f,m_hitPos[2]+s);
...
void Sample_TempObstacles::handleSettings()
{
	...
	if (m_geom)
	{
		...
		_snprintf(text, 64, "Tiles  %d x %d", tw, th);
		imguiValue(text);
		...
		_snprintf(text, 64, "Max Tiles  %d", m_maxTiles);
		imguiValue(text);
		_snprintf(text, 64, "Max Polys  %d", m_maxPolysPerTile);
		imguiValue(text);
		gridSize = tw*th;
	}
	else
	...
	_snprintf(msg, 64, "Layers  %d", m_cacheLayerCount);
	imguiValue(msg);
	_snprintf(msg, 64, "Layers (per tile)  %.1f", (float)m_cacheLayerCount/(float)gridSize);
	imguiValue(msg);
	
	_snprintf(msg, 64, "Memory  %.1f kB / %.1f kB (%.1f%%)", m_cacheCompressedSize/1024.0f, m_cacheRawSize/1024.0f, compressionRatio*100.0f);
	imguiValue(msg);
	_snprintf(msg, 64, "Navmesh Build Time  %.1f ms", m_cacheBuildTimeMs);
	imguiValue(msg);
	_snprintf(msg, 64, "Build Peak Mem Usage  %.1f kB", m_cacheBuildMemUsage/1024.0f);
	imguiValue(msg);
...
void Sample_TempObstacles::handleRender()
{
	...
	g_drawInterface->glDepthMask(GL_FALSE);
	...
	g_drawInterface->glDepthMask(GL_TRUE);
...
/*	imguiDrawRect(280,10,300,100,imguiRGBA(0,0,0,64));
	...
	_snprintf(text,64,"Lean Data: %.1fkB", m_tileCache->getRawSize()/1024.0f);
	imguiDrawText(300, y, IMGUI_ALIGN_LEFT, text, imguiRGBA(255,255,255,255));
	y -= 20;
	
	_snprintf(text,64,"Compressed: %.1fkB (%.1f%%)", m_tileCache->getCompressedSize()/1024.0f,
	...
	{
		_snprintf(text,64,"Changed obstacles, rebuild %d tiles: %.3f ms", m_rebuildTileCount, m_rebuildTime);
...
bool Sample_TempObstacles::handleBuild()
{
	...
	cfg.walkableHeight = (int)Math::ceilf(m_agentHeight / cfg.ch);
	cfg.walkableClimb = (int)Math::floorf(m_agentMaxClimb / cfg.ch);
	cfg.walkableRadius = (int)Math::ceilf(m_agentRadius / cfg.cs);
