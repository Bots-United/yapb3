	...
	m_run(true)
{
	...
	m_toolParams.m_maximumAgentSpeed = HalfLifeEngine::SDK::Constants::MaximumDefaultPlayerSpeed/*3.5f*/;	// EDWARD
...
void CrowdToolState::handleRender()
{
	...
	{
		float gridy = -Math::MaximumFloatValue;
		for (int i = 0; i < crowd->getAgentCount(); ++i)
		{
			const dtCrowdAgent* ag = crowd->getAgent(i);
				...
				for (int i = 0; i < pool->getHashSize(); ++i)
				{
					for (dtNodeIndex j = pool->getFirst(i); j != DT_NULL_IDX; j = pool->getNext(j))
					{
									   ...
									   model, proj, view, &x, &y, &z))
						{
							...
							_snprintf(label, 32, "%.2f", heuristic);
				...
				if (m_toolParams.m_showNeis)
				{
					for (int j = 0; j < ag->nneis; ++j)
					{
						...
						if (gluProject((GLdouble)nei->npos[0], (GLdouble)nei->npos[1]+radius, (GLdouble)nei->npos[2],
									   model, proj, view, &x, &y, &z))
						{
							_snprintf(label, 32, "%.3f", ag->neis[j].dist);
...
void CrowdToolState::addAgent(const float* p)
{
	...
	ap.maxSpeed = m_toolParams.m_maximumAgentSpeed;//3.5f;
...
int CrowdToolState::hitTestAgents(const float* s, const float* p)
{
	...
	int isel = -1;
	float tsel = Math::MaximumFloatValue;
...
void CrowdToolState::updateAgentParams()
{
	...
	for (int i = 0; i < crowd->getAgentCount(); ++i)
	{
		...
		params.maxSpeed = m_toolParams.m_maximumAgentSpeed;
...
void CrowdTool::handleMenu()
{
	...
	if (params->m_expandOptions)
	{
		...
		if (imguiSlider("Max Speed", &params->m_maximumAgentSpeed, 3.0f, HalfLifeEngine::SDK::Constants::MaximumPlayerSpeed, 1.0f))
		{
			m_state->updateAgentParams();
		}
