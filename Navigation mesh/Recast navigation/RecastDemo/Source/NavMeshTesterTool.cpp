...
inline bool inRange(const float* v1, const float* v2, const float r, const float h)
{
	...
	return (dx*dx + dz*dz) < r*r && Math::fabsf(dy) < h;
}
...
NavMeshTesterTool::NavMeshTesterTool() :
	...
	m_toolMode(/*!TOOLMODE_PATHFIND_FOLLOW - CHANGED BY EDWARD*/TOOLMODE_PATHFIND_STRAIGHT),
	m_startRef(0),
	m_endRef(0),
	m_npolys(0),
	m_nstraightPath(0),
...
void NavMeshTesterTool::handleToggle()
{
	...
	len = Math::sqrtf(dtVdot(delta,delta));
...
void NavMeshTesterTool::recalc()
{
	...
	else if (m_toolMode == TOOLMODE_FIND_POLYS_IN_CIRCLE)
	{
		if (m_sposSet && m_startRef && m_eposSet)
		{
			...
			float dist = Math::sqrtf(dx*dx + dz*dz);
#ifdef DUMP_REQS
...
void NavMeshTesterTool::handleRender()
{
	...
	else if (m_toolMode == TOOLMODE_FIND_POLYS_IN_CIRCLE)
	{
		...
		if (m_sposSet && m_eposSet)
		{
			...
			const float dx = m_epos[0] - m_spos[0];
			const float dz = m_epos[2] - m_spos[2];
			const float dist = Math::sqrtf(dx*dx + dz*dz);
