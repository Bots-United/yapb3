...
bool rcMergePolyMeshes(rcContext* ctx, rcPolyMesh** meshes, const int nmeshes, rcPolyMesh& mesh)
{
	...
	for (int i = 0; i < nmeshes; ++i)
	{
		...
		const unsigned short ox = (unsigned short)Math::floorf((pmesh->bmin[0]-mesh.bmin[0])/mesh.cs+0.5f);
		const unsigned short oz = (unsigned short)Math::floorf((pmesh->bmin[2]-mesh.bmin[2])/mesh.cs+0.5f);
