...
{
	return Math::sqrtf(vdistSq2(p,q));
}
...
static float distPtTri(const float* p, const float* a, const float* b, const float* c)
{
	...
	if (u >= -EPS && v >= -EPS && (u+v) <= 1+EPS)
	{
		...
		return Math::fabsf(y-p[1]);
	}
	return Math::MaximumFloatValue;
}
...
static float distToTriMesh(const float* p, const float* verts, const int /*nverts*/, const int* tris, const int ntris)
{
	...
	if (dmin == Math::MaximumFloatValue) return -1;
	return dmin;
}
...
{
	int ix = (int)Math::floorf(fx*ics + 0.01f);
	int iz = (int)Math::floorf(fz*ics + 0.01f);
	...
	if (h == RC_UNSET_HEIGHT)
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			...
			const float d = Math::fabsf(nh*ch - fy);
			if (d < dmin)
			{
				h = nh;
				dmin = d;
							...
							rcIntArray& edges, rcIntArray& samples)
{
	...
	if (sampleDist > 0)
	{
		for (int i = 0, j = nin-1; i < nin; j=i++)
		{
			...
			float dy = vi[1] - vj[1];
			float dz = vi[2] - vj[2];
			float d = Math::sqrtf(dx*dx + dz*dz);
			int nn = 1 + (int)Math::floorf(d/sampleDist);
	...
	if (sampleDist > 0)
	{
		...
		int x0 = (int)Math::floorf(bmin[0]/sampleDist);
		int x1 = (int)Math::ceilf(bmax[0]/sampleDist);
		int z0 = (int)Math::floorf(bmin[2]/sampleDist);
		int z1 = (int)Math::ceilf(bmax[2]/sampleDist);
		samples.resize(0);
		for (int z = z0; z < z1; ++z)
		{
			for (int x = x0; x < x1; ++x)
