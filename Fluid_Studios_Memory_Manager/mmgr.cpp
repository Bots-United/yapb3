// ---------------------------------------------------------------------------------------------------------------------------------
//  _ __ ___   _ __ ___    __ _  _ __      ___  ____   ____
// | '_ ` _ \ | '_ ` _ \  / _` || '__|    / __||  _ \ |  _ \
// | | | | | || | | | | |( (_| || |    _ ( (__ | |_) )| |_) )
...
// ---------------------------------------------------------------------------------------------------------------------------------
#ifdef STRESS_TEST
	static const unsigned char  hashBits             = 12u;
	...
	static const unsigned short paddingSize          = 1024u;	// An extra 8K per allocation!
#else
	static const unsigned char  hashBits             = 12u;
	...
	static bool                 alwaysWipeAll        = true;
	static bool                 cleanupLogOnFirstRun = true;
	static const unsigned short paddingSize          = 4u;
#endif
...
//		#include <assert.h>
		...
		#define	m_assert(cond) InternalAssert (cond)//assert (cond)*/{ if ((cond) == false) DEBUGGER_BREAK (); }
	#else
		#define	m_assert(cond) /* Empty */
	#endif
#elif defined (__BEOS__)
	#ifdef DEBUG
		extern void debugger (const char *message);

		#define	m_assert(cond) { if ((cond) == false) debugger ("mmgr: assert failed") }
	#else
		#define m_assert(cond) /* Empty */
	#endif
...
// Other locals
// ---------------------------------------------------------------------------------------------------------------------------------
static const unsigned int hashSize = BIT (hashBits);
static const char *const allocationTypes[MemoryAllocationType_Total] = {"Unknown", "new", "new[]", "malloc", "calloc", "realloc", "delete", "delete[]", "free"};
static AllocUnit_t *hashTable[hashSize];
static AllocUnit_t *reservoir;
static unsigned int currentAllocationCount = 0u;
static unsigned int breakOnAllocationCount = 0u;
static MemoryStatistics_t stats;
static const char *sourceFile = "??";
static const char *sourceFunc = "??";
static unsigned int sourceLine = 0u;
static bool staticDeinitTime = false;
static AllocUnit_t **reservoirBuffer = NULL;
static unsigned int reservoirBufferSize = 0u;
static const char *const memoryLogFile = "memory.log";
static const char *const memoryLeakLogFile = "memleaks.log";
...
static void log (const char *const format, ...)
{
	...
	// Open the log file
	STDIOFile file (memoryLogFile, "ab");
...
static void doCleanupLogOnFirstRun (void)
{
	...
	log ("--------------------------------------------------------------------------------");
	log ("");
	log ("      %s - Memory logging file created on %s", memoryLogFile, asctime (GetLocalTime ()));
	log ("--------------------------------------------------------------------------------");
	log ("");
...
static inline const char *const sourceFileStripper (const char *const sourceFile)
{
...
static inline const char *const ownerString (const char *const sourceFile, const unsigned int sourceLine, const char *const sourceFunc)
{
	static char str[178u];

	memset (str, 0, sizeof (str));

	sprintf (str, "%s (%u)::%s", sourceFileStripper (sourceFile), sourceLine, sourceFunc);
...
static const char *const insertCommas (const unsigned int value)
{
	static char str[30u];
	...
	if (GetStringLength <unsigned short> (str) > 3u)
	{
		memmove (str + (GetStringLength <unsigned short> (str) - 3u), str + (GetStringLength <unsigned short> (str) - 4u), 4);

		str[GetStringLength <unsigned short> (str) - 4] = ',';
	}

	if (GetStringLength <unsigned short> (str) > 7u)
	{
		memmove (str + (GetStringLength <unsigned short> (str) - 7u), str + (GetStringLength <unsigned short> (str) - 8u), 8);

		str[GetStringLength <unsigned short> (str) - 8u] = ',';
	}

	if (GetStringLength <unsigned short> (str) > 11u)
	{
		memmove (str + (GetStringLength <unsigned short> (str) - 11u), str + (GetStringLength <unsigned short> (str) - 12u), 12);

		str[GetStringLength <unsigned short> (str) - 12u] = ',';
	}
...
static inline const char *const memorySizeString (const unsigned int size)
{
	static char str[90u];

	if (size > MATH_GET_SQUARED (1024u))
		sprintf (str, "%10s (%7.2fM)", insertCommas (size), static_cast <float> (size) / MATH_GET_SQUARED (1024.0f));
	else if (size > 1024u)
...
AllocUnit_t *const m_findAllocUnit (const void *const reportedAddress)
{
	...
	// very good coverage.
	const unsigned int hashIndex ((reinterpret_cast <const unsigned int> (reportedAddress) >> 4u) & (hashSize - 1u));

	for (AllocUnit_t *ptr (hashTable[hashIndex]); ptr != NULL; ptr = ptr->next)
...
static inline const size_t calculateActualSize (const size_t reportedSize)
{
	// We use DWORDS as our padding, and a int is guaranteed to be 4 bytes, but an int is not (ANSI defines an int as
	// being the standard word size for a processor; on a 32-bit machine, that's 4 bytes, but on a 64-bit machine, it's
	// 8 bytes, which means an int can actually be larger than a int).
...
static inline const size_t calculateReportedSize (const size_t actualSize)
{
	// We use DWORDS as our padding, and a int is guaranteed to be 4 bytes, but an int is not (ANSI defines an int as
	// being the standard word size for a processor; on a 32-bit machine, that's 4 bytes, but on a 64-bit machine, it's
	// 8 bytes, which means an int can actually be larger than a int).
...
static inline void *const calculateReportedAddress (void *const actualAddress)
{
	// We allow this...
	...
	return static_cast <void *const> (static_cast <char *const> (actualAddress) + sizeof (int) * paddingSize);
}

// ---------------------------------------------------------------------------------------------------------------------------------

static void wipeWithPattern (const AllocUnit_t *const allocUnit, unsigned int pattern, const unsigned int originalReportedSize = 0u)
{
...
//	pattern = 0ul;
	...
	{
		// Fill the bulk
		int *lptr (reinterpret_cast <int *> (static_cast <char *> (allocUnit->reportedAddress) + originalReportedSize));
		...
		// Fill the remainder
		unsigned int shiftCount (0u);
		char *cptr (reinterpret_cast <char *> (lptr));

		for (i = 0; i < (length & 0x3); ++i, ++cptr, shiftCount += 8u)
			*cptr = static_cast <char> ((pattern & (0xff << shiftCount)) >> shiftCount);
	}

	// Write in the prefix/postfix bytes
	int *pre (static_cast <int *> (allocUnit->actualAddress));
	int *post (reinterpret_cast <int *> (static_cast <char *> (allocUnit->actualAddress) + allocUnit->actualSize - paddingSize * sizeof (int)));

	for (unsigned int index (0u); index < paddingSize; ++index, ++pre, ++post)
	{
		*pre = prefixPattern;
		*post = postfixPattern;
	}
...
static void dumpAllocations (FILE *const fp)
{
	fprintf (fp, "Alloc.  Address      Size     Address      Size                        BreakOn BreakOn\n");
	...
	for (unsigned int index (0u); index < hashSize; ++index)
		for (const AllocUnit_t *ptr (hashTable[index]); ptr != NULL; ptr = ptr->next)
		{
			if (strcmp (ptr->sourceFunc, "CoreToolLibrary::DynamicArray<char,unsigned short>::SetSizeWithoutKeepingData") == 0)
				fprintf (fp, "\"%s\" - ", static_cast <char *> (ptr->reportedAddress));
...
static void dumpLeakReport (void)
{
	...
	FILE *const fp (fopen (memoryLeakLogFile, "w+b"));

	// If you hit this assert, then the memory report generator is unable to log information to a file (can't open the file for some reason).
	m_assert (fp != NULL);
	...
	fprintf (fp, " ----------------------------------------------------------------------------------------------------------------------------------\n");
	fprintf (fp, "|                                          Memory leak report for: %02d/%02d/%04d %02d:%02d:%02d                                             |\n", localTime->tm_mday, localTime->tm_mon + 1u, localTime->tm_year + 1900u, localTime->tm_hour, localTime->tm_min, localTime->tm_sec);
	fprintf (fp, " ----------------------------------------------------------------------------------------------------------------------------------\n\n\n");

	// Any leaks?
	if (stats.totalAllocUnitCount > 0u)
	{
		fprintf (fp, "%u memory leak%s found:\n\n", stats.totalAllocUnitCount, stats.totalAllocUnitCount == 1u ? "" : "s");
	...
	else
	{
		...
		{
			for (unsigned int i (0u); i < reservoirBufferSize; ++i)
...
{
	public:
		inline  MemStaticTimeTracker (void) { doCleanupLogOnFirstRun (); }
		inline ~MemStaticTimeTracker (void) { staticDeinitTime = true; dumpLeakReport (); }
};
static const MemStaticTimeTracker mstt;
...
bool &m_breakOnRealloc (const void *const reportedAddress)
{
	...
	AllocUnit_t *const au (m_findAllocUnit (reportedAddress));
	...
	m_assert (au->allocationType == MemoryAllocationType_Malloc || au->allocationType == MemoryAllocationType_Calloc || au->allocationType == MemoryAllocationType_Realloc);
...
void m_breakOnAllocation (const unsigned int count)
{
...
static inline void resetGlobals (void)
{
	sourceFile = "??";
	sourceLine = 0;
	sourceFunc = "??";
...
void *operator new (size_t reportedSize)
{
	...
	const char *const file (sourceFile);
	const unsigned int line (sourceLine);
	const char *const func (sourceFunc);
	...
	{
		// Try the allocation
		void *const ptr (m_allocator (file, line, func, MemoryAllocationType_New, reportedSize));
...
void *operator new [] (size_t reportedSize)
{
	...
	{
		// Try the allocation
		void *const ptr (m_allocator (file, line, func, MemoryAllocationType_NewArray, reportedSize));
...
void *operator new (size_t reportedSize, const char *const sourceFile, const int sourceLine)
{
	...
	{
		// Try the allocation
		void *const ptr (m_allocator (sourceFile, sourceLine, "??", MemoryAllocationType_New, reportedSize));
...
void *operator new [] (size_t reportedSize, const char *const sourceFile, const int sourceLine)
{
	...
	{
		// Try the allocation
		void *const ptr (m_allocator (sourceFile, sourceLine, "??", MemoryAllocationType_NewArray, reportedSize));
...
void operator delete (void *const reportedAddress)
{
	...
	if (reportedAddress != NULL)
		m_deallocator (sourceFile, sourceLine, sourceFunc, MemoryAllocationType_Delete, reportedAddress);
	else if (alwaysLogAll)
		log ("[-] ----- %8s of NULL                      by %s", allocationTypes[MemoryAllocationType_Delete], ownerString (sourceFile, sourceLine, sourceFunc));
...
void operator delete [] (void *const reportedAddress)
{
	...
	if (reportedAddress != NULL)
		m_deallocator (sourceFile, sourceLine, sourceFunc, MemoryAllocationType_DeleteArray, reportedAddress);
	else if (alwaysLogAll)
		log ("[-] ----- %8s of NULL                      by %s", allocationTypes[MemoryAllocationType_DeleteArray], ownerString (sourceFile, sourceLine, sourceFunc));
...
void *const m_allocator (const char *const sourceFile, const unsigned int sourceLine, const char *const sourceFunc, const MemoryAllocationType_t allocationType, const size_t reportedSize)
{
	try
	{
		...
		// Log the request
		if (alwaysLogAll)
			log ("[+] %05d %8s of size 0x%08X (%08d) by %s", currentAllocationCount, allocationTypes[allocationType], reportedSize, reportedSize, ownerString (sourceFile, sourceLine, sourceFunc));
		...
		if (reservoir == NULL)
		{
			...
			reservoir = static_cast <AllocUnit_t *> (malloc (sizeof (AllocUnit_t) * 256u));

			// If you hit this assert, then the memory manager failed to allocate internal memory for tracking the allocations
			...
			memset (reservoir, 0, sizeof (AllocUnit_t) * 256u);

			for (unsigned int i (0u); i < 256u - 1u; ++i)
				reservoir[i].next = reservoir + (i + 1u);

			// Add this address to our reservoirBuffer so we can free it later

			AllocUnit_t **const temp = static_cast <AllocUnit_t **> (realloc (reservoirBuffer, (reservoirBufferSize + 1) * sizeof (AllocUnit_t *)));
		...
		AllocUnit_t *const au (reservoir);
		reservoir = au->next;

		// Populate it with some real data

		memset (au, 0, sizeof (AllocUnit_t));

		au->actualSize = calculateActualSize (reportedSize);

		#ifdef RANDOM_FAILURE
			const double a (rand ());
			const double b (RAND_MAX / 100.0 * RANDOM_FAILURE);
		...
		if (sourceFile != NULL)
			strncpy (au->sourceFile, sourceFileStripper (sourceFile), sizeof (au->sourceFile));
		else
			strcpy (au->sourceFile, "??");

		if (sourceFunc != NULL)
			strncpy (au->sourceFunc, sourceFunc, sizeof (au->sourceFunc));
		else
		...
		m_assert (allocationType != MemoryAllocationType_Unknown);

		// Insert the new allocation into the hash table

		const unsigned int hashIndex ((reinterpret_cast <unsigned int> (au->reportedAddress) >> 4) & (hashSize - 1u));
		...
		if (allocationType == MemoryAllocationType_Calloc)
...
void *const m_reallocator (const char *const sourceFile, const unsigned int sourceLine, const char *const sourceFunc, const MemoryAllocationType_t reallocationType, const size_t reportedSize, const void *const reportedAddress)
{
	try
	{
		...
		// Log the request
		if (alwaysLogAll)
			log ("[~] %05d %8s of size 0x%08X (%08d) by %s", currentAllocationCount, allocationTypes[reallocationType], reportedSize, reportedSize, ownerString (sourceFile, sourceLine, sourceFunc));
		...
		m_assert (reallocationType != MemoryAllocationType_Unknown);
		...
		const unsigned int originalReportedSize (static_cast <unsigned int> (au->reportedSize));
		...
		const void *const oldReportedAddress (reportedAddress);
		const size_t newActualSize (calculateActualSize (reportedSize));
		...
		// We don't want to assert with random failures, because we want the application to deal with them.
			...
			m_assert (newActualAddress != NULL);
		#endif
		...
		if (oldReportedAddress != au->reportedAddress)
		{
			...
			unsigned int hashIndex ((reinterpret_cast <unsigned int> (oldReportedAddress) >> 4) & (hashSize - 1u));
			...
			hashIndex = (reinterpret_cast <unsigned int> (au->reportedAddress) >> 4) & (hashSize - 1u);
		...
		const int deltaReportedSize (static_cast <int> (reportedSize - originalReportedSize));
...
void m_deallocator (const char *const sourceFile, const unsigned int sourceLine, const char *const sourceFunc, const MemoryAllocationType_t deallocationType, const void *const reportedAddress)
{
	try
	{
		...
		// Log the request
		if (alwaysLogAll)
			log ("[-] ----- %8s of addr 0x%08X           by %s", allocationTypes[deallocationType], reinterpret_cast <const unsigned int> (reportedAddress), ownerString (sourceFile, sourceLine, sourceFunc));
...
//		m_assert (au != NULL);
		if (au == NULL)
			AddLogEntry (false, LogLevel_Critical, true, "m_deallocator(): Request to deallocate RAM that was never allocated!\n\tFile: '%s',\n\tline: %u,\n\tfunction: %s.", sourceFile, sourceLine, sourceFunc);
		...
		// tracking software, use the stack frame to locate the source and include our H file.
		m_assert (deallocationType != MemoryAllocationType_Unknown);
		...
		m_assert
		(
			(deallocationType == MemoryAllocationType_Delete && au->allocationType == MemoryAllocationType_New) ||
			(deallocationType == MemoryAllocationType_DeleteArray && au->allocationType == MemoryAllocationType_NewArray) ||
			(deallocationType == MemoryAllocationType_Free && au->allocationType == MemoryAllocationType_Malloc) ||
			(deallocationType == MemoryAllocationType_Free && au->allocationType == MemoryAllocationType_Calloc) ||
			(deallocationType == MemoryAllocationType_Free && au->allocationType == MemoryAllocationType_Realloc) ||
			deallocationType == MemoryAllocationType_Unknown
		);
		...
		const unsigned int hashIndex ((reinterpret_cast <unsigned int> (au->reportedAddress) >> 4u) & (hashSize - 1u));
...
// ---------------------------------------------------------------------------------------------------------------------------------
// -DOC- The following utilitarian allow you to become proactive in tracking your own memory, or help you narrow in on those tough bugs.
// ---------------------------------------------------------------------------------------------------------------------------------
...
const bool m_validateAddress (const void *const reportedAddress)
{
	...
	return m_findAllocUnit (reportedAddress) != NULL;
}
...
const bool m_validateAllocUnit (const AllocUnit_t *const allocUnit)
{
	...
	const int *pre (static_cast <int *> (allocUnit->actualAddress));
	const int *post (reinterpret_cast <int *> (static_cast <char *> (allocUnit->actualAddress) + allocUnit->actualSize - paddingSize * sizeof (int)));
	bool errorFlag (false);

	for (unsigned int i (0u); i < paddingSize; ++i, ++pre, ++post)
	{
		if (*pre != static_cast <int> (prefixPattern))
		{
		...
		m_assert (*pre == static_cast <int> (prefixPattern));

		if (*post != static_cast <int> (postfixPattern))
		{
		...
		m_assert (*post == static_cast <int> (postfixPattern));
	}
...
const bool m_validateAllAllocUnits (void)
{
	...
	if (errors > 0u)
		log ("[!] While validting all allocation units, %u allocation unit (s) were found to have problems", errors);
...
{
	const unsigned int *ptr (static_cast <unsigned int *> (allocUnit->reportedAddress));
	unsigned int count (0u);

	for (unsigned int index (0u); index < allocUnit->reportedSize; index += sizeof (int), ++ptr)
...
const unsigned int m_calcAllUnused (void)
{
	...
	for (unsigned int index (0u); index < hashSize; ++index)
		for (const AllocUnit_t *ptr (hashTable[index]); ptr != NULL; total += m_calcUnused (ptr), ptr = ptr->next);
...
void m_dumpMemoryReport (const char *const filename/* = "memreport.log"*/, const bool overwrite/* = true*/)
{
	...
	FILE *const fp (fopen (filename, overwrite ? "w+b" : "ab"));
