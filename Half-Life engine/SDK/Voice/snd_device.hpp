...
public:
	...
	virtual void		SpatializeChannel( int volume[4], int master_vol, const Math::Vector3D& sourceDir, float gain, float dotRight, float dotFront ) = 0;
	...
	virtual void		UpdateListener( const Math::Vector3D& position, const Math::Vector3D& forward, const Math::Vector3D& right, const Math::Vector3D& up ) = 0;
