...
void VoiceSE_Idle(float frametime)
{
	...
	{
		g_VoiceOverdriveDuration = min( g_VoiceOverdriveDuration+frametime, Console::Bot::variableManager.GetVariable (Console::Bot::VariableID_Voice_overdrivefadetime)->GetValue <float> () );
	}
	else
	{
	...
	float percent = g_VoiceOverdriveDuration / Console::Bot::variableManager.GetVariable (Console::Bot::VariableID_Voice_overdrivefadetime)->GetValue <float> ();
	percent = (float)(-cos(percent * 3.1415926535) * 0.5 + 0.5);		// Smooth it out..
	float voiceOverdrive = 1 + (Console::Bot::variableManager.GetVariable (Console::Bot::VariableID_Voice_overdrive)->GetValue <float> () - 1) * percent;
